...
export default class CustomRenderer extends BpmnRenderer {
  ...
  drawShape(parentNode, element) {
    ...
    const { optional, extensionElements } = businessObject;
    ...
    if (extensionElements) {
      ...
      let executionModalities = extensionElements.values.find(element => element.$type === 'sustainability:ExecutionModalities')

      let multiple = ['highPerformance', 'standard', 'lowPower'].filter((modality) => (modality in executionModalities)).length > 1;
